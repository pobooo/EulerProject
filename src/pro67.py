...
for i in range(1,len(path)):
    path_val[0]=last_path_val[0]+int(path[i][0])
    path_val[i]=last_path_val[i-1]+int(path[i][i])
    for j in range(1,i):
            val1=last_path_val[j-1]+int(path[i][j])
            val2=last_path_val[j]+int(path[i][j])
            if val1<val2:
                path_val[j]=val2
                #path_patten[i-1][j]=1
            else:
    ...
    path_val=[0]*len(path)#不加这句就错了 不知为什么
